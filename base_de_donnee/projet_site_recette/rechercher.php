    ...
    <body>
        ...
        <main>
            ...
            try {
                ...
                $critereDeRecherche = $_GET["typeOfSearch"];
                $recherche = $_GET["searchBar"];
                if ($critereDeRecherche === "nom") {
                    $nomRecette = "%" . $recherche . "%";
                    
                    $sql = "SELECT idRecette, nom FROM recettes
                            WHERE nom LIKE :nomDeLaRecette";
                    
                    $requete = $dbh->prepare($sql);

                    $requete->bindParam(":nomDeLaRecette", $nomRecette, PDO::PARAM_STR);

                    $requete->execute();
                } elseif ($critereDeRecherche === "ingredients") {
                    ...
                    $requete->execute();
                } elseif ($critereDeRecherche === "tempsPreparation") {
                    $tempsDePreparation = intval($recherche);

                    $sql = "SELECT idRecette, nom FROM recettes
                            WHERE tempsDePreparation <= :tempsDePreparation";
